...
- 原则真源在 `.human/architecture.md`（含 DDD 战术与六边形适配器约束）
- 流程真源在 `.ai/workflow-feature-delivery.md`
- 校验执行在 `.skills/`：
...
1. `.human/architecture.md`（架构 + DDD + 六边形约束）
2. `.human/glossary.md`（统一语言）
3. `.ai/index.md`（系统入口）
4. `.ai/current.md`（当前状态）
...
开始实现前，AI MUST 按 `.human/architecture.md` 的 ARCH-R1~R6 逐条自检。
