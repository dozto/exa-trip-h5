...
- `PlanTripRoutes({ tripPlan, dayId, departureTime, strategies }) -> Result<NavigationPlan, AppError>`
- `ShowDayDecisionHints({ tripPlan, dayId, navigationPlan, strategy, defaultBufferMinutes }) -> Result<DayDecisionHints, AppError>`
- 视图交互命令（驱动侧事件总线，非 use case port）：
  - `strategySelected(strategy)`：切换当前路线策略。
  - `placeSelected(placeId)`：进入景点聚焦视图。
  - `viewEscaped()`：逐级退出（place → day → overview）。
...
4. 调用 `PlanTripRoutes`（按当前 `selectedStrategy` 传入 `strategies`）产出当天多策略路线候选。
5. 调用 `ShowDayDecisionHints` 产出活动级事件预估与衔接可行性。
6. 调用 `SwitchCurrentDay` 输入 `dayId`，在切换后重复步骤 4-5。
7. 视图层按三层视图模型（`overview` / `day` / `place`）+ `selectedStrategy` 渲染：概览卡 / 当日行程卡（含逐 leg 时间与距离）/ 景点聚焦卡（前序/后继 leg + 出发建议）。
8. `strategySelected` / `placeSelected` / `viewEscaped` 交互命令由 handlers 编排，前者触发重算路线，后两者仅切视图状态。
...
- RFC：`.ai/rfcs/RFC-0003-decision-hints-and-view-orchestration.md`、`.ai/rfcs/RFC-0004-place-focus-view-and-strategy-routing.md`
...
- 三层视图模型（`overview/day/place`）属 inbound 状态，不进入 domain。
